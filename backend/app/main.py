...
from app.models import User, UserPreference, KanbanColumn, Tag, SubTask, Comment, Notification, repair_card_tags, RepairCardMedia
...
def _setup_observability(app: FastAPI, settings) -> None:
    if settings.sentry_dsn:
        try:
            ...
            import sys
            sys.stderr.write(f"Warning: Sentry initialization failed: {e}\n")
    ...
    if settings.enable_prometheus_metrics:
        try:
            ...
            import sys
            sys.stderr.write(f"Warning: Prometheus instrumentation failed: {e}\n")
...
async def lifespan(app: FastAPI):
    ...
    # Migración runtime opcional (deshabilitada por defecto y no recomendada en producción)
    if settings.runtime_schema_migration and not settings.is_production:
